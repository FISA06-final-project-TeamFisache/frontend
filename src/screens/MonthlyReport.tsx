...
import { getReport, generateReport, type ReportDetail } from '../api/reportApi';
...
const DEFAULT_DATA = {
  totalIncome:  0,
  totalExpense: 0,
  expenseCategories: [] as { category: string; value: number }[],
  nextMonthGuideline: '',
  marketCondition: '',
  performanceComment: '',
  performanceStatus: '',
...
export default function MonthlyReport() {
  ...
  // API 데이터 → 화면 데이터 매핑
  const expenseCategories: { category: string; value: number }[] = (() => {
    if (!report?.expenseCategories) return DEFAULT_DATA.expenseCategories;
    const cats = report.expenseCategories as { categoryName?: string; expenseAmount?: number; category?: string; value?: number }[];
    return cats.map(c => ({
      category: c.categoryName ?? c.category ?? '기타',
      value: c.expenseAmount ?? c.value ?? 0,
    }));
  })();

  const data = {
    totalIncome:       report?.totalIncome       ?? DEFAULT_DATA.totalIncome,
    totalExpense:      report?.totalExpense      ?? DEFAULT_DATA.totalExpense,
    expenseCategories: expenseCategories.length > 0 ? expenseCategories : DEFAULT_DATA.expenseCategories,
    nextMonthGuideline: report?.nextMonthGuideline ?? DEFAULT_DATA.nextMonthGuideline,
    marketCondition:   (report as ReportDetail & { marketCondition?: string })?.marketCondition ?? DEFAULT_DATA.marketCondition,
    performanceComment: (report as ReportDetail & { performanceComment?: string })?.performanceComment ?? DEFAULT_DATA.performanceComment,
    performanceStatus:  (report as ReportDetail & { performanceStatus?: string })?.performanceStatus  ?? DEFAULT_DATA.performanceStatus,
  };
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 52px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {data.marketCondition && (
            <Card>
              ...
              <p style={{ fontSize: 12, color: '#475569', lineHeight: 1.8, margin: 0 }}>{data.marketCondition}</p>
            </Card>
          )}
          ...
          <Card>
            ...
            <div style={{ marginTop: 10, padding: '10px 12px', background: '#f8fafc', borderRadius: 10, fontSize: 12, color: '#475569', lineHeight: 1.8 }}>
              {data.performanceComment || MOCK_BENCHMARK_COMMENT}
            </div>
          </Card>
