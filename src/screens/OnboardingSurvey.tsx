...
type OnboardingSurveyProps = {
  ...
  onSurveyAnswer: (score: number) => void;
...
export default function OnboardingSurvey({
  surveyStep, surveyAnswers, routingFlowContext,
  availableAccounts,
  onSurveyAnswer, setRoutingSetup, setAppState, setRoutingFlowContext,
          ...
          {surveyStep === 0 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right duration-300">
              ...
              <div className="space-y-3 mt-8">
                <button onClick={() => onSurveyAnswer(1)} className="w-full p-5 border-2 border-gray-200 rounded-2xl text-left hover:border-purple-500 hover:bg-purple-50 transition shadow-sm active:scale-95">
                  <p className="font-bold text-gray-800 text-sm">바겐세일이다! 오히려 추가 매수한다.</p>
                </button>
                <button onClick={() => onSurveyAnswer(0)} className="w-full p-5 border-2 border-gray-200 rounded-2xl text-left hover:border-purple-500 hover:bg-purple-50 transition shadow-sm active:scale-95">
          ...
          {surveyStep === 2 && (
            <div className="space-y-6 animate-in zoom-in-95 duration-300 text-center flex flex-col h-full">
              ...
              <div className="mt-auto pt-4 shrink-0 space-y-3">
                ...
                <button onClick={() => setAppState('account_setup')} className="w-full py-4 rounded-xl font-bold text-gray-500 hover:text-gray-700 transition underline underline-offset-2 text-sm active:scale-95">
                  추천 건너뛰고 내가 직접 설정할게요
                </button>
              </div>
            </div>
